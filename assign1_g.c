...

{
  ...
  printf("CS 2123 Assignment 1 written by Grecia Gonzalez.\n");
  ...
  int lineCount=0, studentCount = 0, i = 1, ageTotal = 0, nextId, nextAge;

  float nextGPA, gpaTotal=0;
...
//Allocates memory of students, with corresponding ID,Name,Age,and GPA  
...

  {
    ...
    fscanf(wordFile, "%d %f", &nextAge, &nextGPA);

    lineCount= lineCount+4;

    array = realloc(array, i * sizeof(Student *));

    *(array + studentCount) = makeStudent(nextId, nextName, lineCount);

     if (*(array+studentCount) != NULL)

     {

       if (setAge(*(array+(studentCount)), nextAge, lineCount) && setGPA(*(array+(studentCount)), nextGPA, lineCount))
